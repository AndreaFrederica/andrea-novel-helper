...
### ✨ 新增
- 新增 `scripts/macos-vm-build.sh`，用于在 macOS 虚拟机内通过 pixi 准备环境并构建 `darwin-x64` 产物，方便 Intel Mac 无容器场景下本地编译。
- 新增写作统计汇总修复命令 `AndreaNovelHelper.timeStats.repairSummary`，可遍历文件追踪数据库并重建写作统计摘要文件，用于修复历史统计异常。
- 新增设置向导命令 `andrea.openSettingsWizard`，用户可以随时从命令面板重新打开图形化设置向导。

### 🚀 优化
- 写作统计仪表盘的全文统计和累计用时现在会叠加当前正在编辑文件的实时会话数据，不再必须等实时统计停止后才刷新。
- 图形化设置向导的首次提示状态改为“用户已明确处理”，只有用户选择暂不使用、关闭向导、应用或保存后才不再自动弹出。
- macOS 虚拟机构建流程补充 pixi / npm / Electron 架构相关处理，降低跨平台打包对 GitHub Actions 容器环境的依赖。

### 🐛 修复
- 修复设置向导在异常关闭、Webview 未加载完成或用户误关面板后也被标记为“已提示”的问题。
- 修复从角色树跳转到 JSON5 可视化角色编辑器时，若刚从 Markdown 角色文件切回已打开的可视化编辑器，第一次可能滚不到目标角色卡的问题。
- 修复写作统计摘要在历史数据异常、路径修复或统计汇总文件不一致时缺少一键重建入口的问题。

## [0.5.2] - 2026-05-09

> 说明：0.5.0 与 0.5.1 未正式发布，因此 0.5.2 是 0.5.x 首个对外发布版本。What's New for 0.5.2 会完整包含 0.5.0 的全部大型更新内容，并追加本轮体验改进。

### ✨ 新增
- 图形化快速设置新增交互式设置向导：先选择写作场景，再按“编辑器显示 / 智能编辑 / 补全与查询键 / 角色列表 / AI 与错字系统 / 状态栏”逐模块设置，最后支持取消、暂存和保存。
- 首次打开当前版本的图形化快速设置时，会询问用户写入项目设置还是全局设置，并询问是否使用设置向导。
- 设置向导新增写作场景预设：通用写作、专注起草、角色密集、AI 校对。场景只决定预设值，不会直接保存设置。
- 图形化快速设置新增 Markdown 描述渲染组件，支持正确显示设置说明中的粗体、行内代码、列表和代码块。
- 新增 LLM 模型选择器，支持从兼容 API 自动拉取模型列表，也允许手动输入自定义模型名。
- 自动补齐括号对、里程碑目标、角色自定义分组等数组/对象类设置支持图形化编辑。
- AI、错字系统、查询键生成、包管理器行为、VS Code 托管禁用、状态栏显示等更多设置加入图形化快速设置。
- 角色列表设置新增独立预览组件，并区分当前文章角色、全部角色和包管理器角色节点的隔离配置。
- 角色列表新增角色类型排序设置，当前文章角色和全部角色都可自定义“主角 / 配角 / 反派”等类型的显示顺序。
- 角色详情新增折行开关；路径、URI、UUID、正则、SVG/图标等字段始终保持不折行。
- 角色卡 JSON5 可视化编辑器新增表格化 UI 组件，并支持新老 UI 切换。
- 随机角色生成器拆分为 Vue 组件并接入角色编辑器。
- 角色卡编辑器新增字段显示名本地化开关，可在中文、英文、日文显示名与原始 JSON key 之间切换；保存时仍保留真实 key。
- 角色卡编辑器支持从后端请求拼音 / 罗马字查询键候选，并可多选后批量追加到当前角色字段。
- Webview 前端新增中文/英文 i18n 结构，快速设置相关组件可跟随 VS Code 语言环境逐步本地化。

### 🚀 优化
- 设置向导预设按小说写作习惯重做：所有场景的 `editor.wrappingIndent` 默认使用 `none`，中文写作缩进宽度默认使用 `4`。
- “通用写作”预设保持保守；“专注起草”只调整编辑器、智能编辑和状态栏；“角色密集”只强化补全与角色列表；“AI 校对”只启用错字/LLM 相关模块。
- AI 校对预设不默认开启自动打开扫描和变更扫描，避免在用户未明确确认前发送文档内容。
- 每个快速设置项显示当前值来源，可区分默认、全局和工作区设置；布尔项也显示当前是否显式设置。
- 图形化快速设置为角色类型排序提供可视化列表编辑器，支持预设、添加、删除、上移、下移和恢复默认。
- 角色卡表格编辑器合并颜色与样式编辑区域，补充字段显示名、原始 key 提示、扩展字段选择和 UUID 复制反馈。
- 角色卡经典编辑器、侧栏、查找候选弹窗和随机生成器统一接入 Webview i18n 与 VS Code 主题变量。
- 随机角色草案会补充拼写、罗马字和适用的拼音查询键，方便生成后直接参与补全、匹配和引用检索。
- 字体家族预览增加分页，避免示例句子过多导致卡片显示不下。
- 段落排版、状态栏、角色列表等预览布局继续收敛，减少卡片堆叠和无效占位。

### 🐛 修复
- 修复设置向导遮罩和 CSS 布局导致的移动闪烁、标题溢出、内容不能滚动、底部按钮间距异常等问题。
- 修复智能编辑自动补全括号预览中光标位置错误，补全后光标会落在括号内部。
- 修复快速设置保存按钮反馈不明显的问题，保存后会更新状态提示和原始设置快照。
- 修复角色详情长路径、URI、UUID 等内容被错误折行的问题。
- 修复角色卡表格编辑器清空样式时前景色字段可能残留的问题。

## [0.5.0] - 2026-05-07
### ✨ 新增
- 新增 MCP HTTP 服务器与 stdio 代理桥，支持 VS Code Copilot Agent Mode、Cursor、Claude Desktop、Continue.dev 等本地 AI 工具读取项目上下文。
- 新增 Copilot Agent Skills / Prompt 文档释放能力，内置 `anh-project`、`anh-script-runtime`、`anh-typst-templates` 等项目辅助指令。
- 新增项目级 MCP 工具，包括角色搜索、当前文档快照、角色使用统计、批注汇总、文档装饰和内置技能文档读取。
- 包管理器新增“常用功能”和“项目设置文件”入口，集中放置项目初始化、项目设置、引用维护、外部资源、Copilot 文档释放、功能引导、文档中心、创作工作台和角色关系图谱。
- 包管理器支持在角色承载文件下直接展示该文件定义的角色，并显示角色数量。
- 角色树与包管理器角色节点支持 SVG 图标、角色颜色色块、详情展开控制和属性折行配置。
- 新增角色查询键系统，支持 `lookupKeys` / `searchKeys` / `queryKeys` / `keywords` / `检索键` / `索引键` 等字段参与补全、正文匹配、引用索引和 AI 检索。
- 支持拼音、日文假名/汉字罗马字、通用罗马字和本地 LLM 转写候选生成；项目配置可声明默认角色索引键和扩展索引键前缀。
- 预览面板支持按角色类型选择着色范围，并新增 Markdown 标题、列表、引用、代码、粗体、斜体、删除线和自定义阅读主题设置。
- 新增图形化项目设置页面，可编辑 `anhproject.md` 和 `project-config.json5`。
- 新增图形化项目初始化向导、交互式功能引导、文档查看器系统和图形化快速设置页面。
- 新增 What's New 更新公告系统，支持版本检测、首次更新自动弹出、历史版本查看、手写 Markdown/HTML 更新页和 CHANGELOG 回退。
- 新用户 walkthrough 从 6 步扩展到 9 步，补充 VS Code 侧边栏、ANH 功能入口、文档中心和快速设置引导。
- 新增创作工作台：集中管理任务、计划、时钟、计时器、日志、年度计划与自由平铺窗口，支持从包管理器常用功能打开。
- 新增时间线甘特图编辑器与创作工作台时间线组件，支持任务/事件的横向规划、同步保存和独立窗口打开。
- 新增任务四象限视图，用于按重要/紧急维度组织写作任务。
- 新增批注管理器 Webview：集中查看、跳转和处理项目批注，并补充批注存储结构。
- 新增角色关系图谱：基于角色库内部引用、角色 `relations` 标记和 `.rjson5/.json5` 关系表生成 Obsidian 风格关系网络。
- 角色关系图谱支持多布局策略、连线显示策略、选中后一跳网络聚焦、悬停高亮邻居、可开关角色 Hover 卡片和可自定义的 Hover 滚轮捕获快捷键。

### 🚀 优化
- 文件追踪、数据库后端、写作统计读取接口全面异步化，减少侧边栏、状态栏和统计面板阻塞。
- SQLite 后端初始化增加最长 15 秒写操作队列，超时自动回退 JSON 分片存储，初始化完成后自动 Drain 排队操作。
- 新增写作统计摘要数据层，提供项目级摘要、单文件摘要、今日统计、24 小时热力图和日粒度历史，供写作统计面板和创作工作台快速读取。
- 新增 `AndreaNovelHelper.fileTracking.repairPathKeys` 命令，可修复绝对/相对路径混用、大小写差异、UUID 冲突和缺失元数据。
- 优化创作工作台热力图、资源映射与 VS Code Webview 资源加载兼容性。
- 更新文件追踪相关设置文案，明确 `.wcignore` 不再影响基础文件追踪，并补充 `respectGitignore` 等替代选项。

### 🐛 修复
- 修复预览面板角色着色偶发失效：Webview 改为主动请求角色颜色和高亮范围，角色数据变化时重新拉取，避免页面重建或脚本尚未加载时丢消息。
- 修复预览角色着色在 DOM 更新后可能不补色或反复重入的问题，并支持按启用角色类型过滤别名、修复词、查询键和正则角色高亮。
- 修复文件创建、删除事件连续触发导致的重复刷新、重复索引和目录统计抖动。
- 修复文件删除、重命名未等待异步持久化完成，导致数据库、摘要和侧边栏短时间不一致的问题。
- 修复 SQLite 后端初始化期间路径映射未预热时可能为已存在文件生成新 UUID 的竞态。
- 修复旧文件追踪索引中的绝对路径、相对路径、Windows 大小写差异和同一路径多 UUID 冲突。
- 修复写作统计摘要在文件删除、重命名、路径修复后可能不一致的问题；摘要失效时会自动标记并防抖重建。
- 修复中文等无空格文本下角色补全只匹配最后一个分词，导致完整角色名无法补全或替换范围错误的问题。
- 修复包管理器拖拽、复制、剪切可能把角色层级节点当作文件路径处理的问题。
- 修复 MCP HTTP 服务器错误响应暴露内部异常/堆栈信息的问题。
- 修复部分 VS Code Webview 环境下工作台、热力图等资源路径映射失败的问题。
...
- 新增Markdown图片渲染支持到预览功能
  - 扩展Webview的localResourceRoots以支持加载工作区和文档目录中的本地图片
  - 修改预览渲染逻辑，支持识别Markdown图片语法 `![alt](src)` 并渲染为HTML `<img>` 标签
  - 添加图片渲染辅助方法，包括路径解析和上下文传递
  - 支持纯图片段落（居中显示）和混合段落（内联显示）
  - 修复md_plain.ts中图片正则与链接正则的处理顺序，确保图片语法正确转换为占位符
  - 为图片添加基础CSS样式，包括最大宽度限制和圆角边框
  - 支持增量更新时图片的实时渲染

### 🐛 修复
- 修复预览功能中Markdown图片无法渲染的问题，现在图片语法会被正确识别并显示为图片而非纯文本

## [0.4.59] - 2026-03-08
### ✨ 新增
- 优化写作资源管理器排序功能的用户体验
  - 文件夹现在会根据其内容排序模式显示不同的视觉标识：手动排序模式显示 `⇅` 标记
  - 右键菜单语义化：自动模式文件夹显示"开启手动排序"，手动模式文件夹显示"关闭手动排序"
  - 悬停提示增强：文件夹tooltip现在会显示内容排序模式状态和使用说明
  - 改进contextValue区分：手动模式文件夹使用独立的 `wordCountFolderManual` contextValue，便于扩展更多功能
- 增强字数统计功能，支持角色文件 (.ojson5)
  - 为角色文件添加专门的计数逻辑，准确统计叙事文本而忽略元数据
  - 引入新的工具函数来识别可计数的角色文件并提取相关文本
  - 更新字数统计提供者以处理资源文件夹标记和刷新逻辑
  - 实现重新扫描文件夹中资源文件的命令，并提供用户反馈
  - 增强外部文件夹扫描以包含新的文件类型和关键词，以更好地管理资源
  - 改进资源文件夹标记的缓存机制以优化性能

### 🐛 修复
- 修复了写作资源管理器在“无可统计文件”场景下，目录节点可能长期停留在 `(计算中...)` 的问题。
- 修复了目录聚合在初始化阶段被硬跳过导致的卡住问题；现在即使初始化未完成，也会继续调度目录聚合并收敛到正确统计值。
- 调整外部资源目录识别规则：普通 `.md` 文件不再参与关键字匹配；新增开关 `AndreaNovelHelper.externalFolder.enableMdLibraryNameMarkers`，仅对角色库命名规则的 `.md` 文件（如 `character-gallery.md`、`*_character.md`）进行可选识别。
- 外部资源扫描报告改为全量输出，不再仅展示节选目录与样例文件。
- 修复角色可视化编辑器的文件类型路由：`.ojson5` 现在会按角色编辑器流程打开并参与同步保存。
- 修复角色可视化编辑器在 `files.autoSave=off` 下的写入语义：变更会立即写入 `TextDocument` 形成未保存修改（dirty），不再误触发强制落盘。
- 放宽角色编辑器保存前校验为 JSON5 语法校验，避免因“顶层结构限制”导致部分合法角色文件无法自动同步。
- 修复角色样式字段（`style`/`color`）前后端转换的覆盖与残留问题：统一颜色回写策略，并在保存时清理旧 `backgroundColor/bold/italic/strikethrough/underline` 残留字段，避免样式编辑后丢数据或回滚。
- 修复背景色清空后偶发无法删除的问题：调整前端提交时序，先应用 `backgroundColor` 删除再提交，确保后端文件中对应字段可被正确移除。

### ⚠️ TODO（同步自 `docs/TODO.md`）
- 当前文章角色视图：实现节点级增量 diff 局部刷新（`docRolesTreeView` / `docRolesExplorerView`），减少动态更新时整树重绘造成的闪动，并保持展开状态稳定。

## [0.4.57] - 2026-01-22
### ✨ 新增
- 新增配置项 `AndreaNovelHelper.useVsCodeManagedDisabling`，允许用户选择完全使用 VSCode 托管的禁用系统来管理插件的启用和禁用状态，提升用户体验和兼容性。

## [0.4.55] - 2026-01-22
### 🐛 修复
- 支持了标准的VSCode插件软禁用功能，现在可以通过VSCode的扩展视图中的禁用按钮来软禁用小说助手插件，而不需要手动修改设置文件。
