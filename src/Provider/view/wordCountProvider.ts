...
export class WordCountProvider implements vscode.TreeDataProvider<WordCountItem | NewItemNode> {
        ...
        for (const d of dirents) {
            ...
            if (d.isDirectory()) {
                ...
                if (cacheValid) {
                    ...
                    // 强制：改回占位符显示“计算中”
                    const zero: TextStats = { cjkChars: 0, asciiChars: 0, words: 0, nonWSChars: 0, nonPunctChars: 0, total: 0 };
                    const item = new WordCountItem(
                        uri,
                        d.name,
                        zero,
    ...
    private async analyzeFolderDynamic(folder: string, exts: string[]): Promise<TextStats> {
        ...
        const work = (async () => {
            let agg: TextStats = { cjkChars: 0, asciiChars: 0, words: 0, nonWSChars: 0, nonPunctChars: 0, total: 0 };
            try {
    ...
    private async getOrCalculateFileStats(filePath: string, forceOverride = false): Promise<TextStats> {
        try {
            ...
            return { cjkChars: 0, asciiChars: 0, words: 0, nonWSChars: 0, nonPunctChars: 0, total: 0 };
        }
    }
    ...
    private reportAggregateDiff(dir: string, agg: TextStats, processedFiles: Set<string>) {
        try {
            ...
            let realSum: TextStats = { cjkChars: 0, asciiChars: 0, words: 0, nonWSChars: 0, nonPunctChars: 0, total: 0 };
