...
export function registerQuickSettings(context: vscode.ExtensionContext, onRefreshStatus: () => void) {
    ...
    async function toggleWordCountUnit() {
        ...
        onRefreshStatus();
        vscode.window.showInformationMessage(`字数单位已切换为：${next === 'excludePunct' ? '不含标点/空白' : '含标点'}`);
    }
    ...
    async function quickSettings() {
        ...
        const pick = await vscode.window.showQuickPick(
            [
                ...
                { label: `${wcUnit === 'excludePunct' ? '$(check)' : '$(circle-slash)'} 切换：字数单位（不含标点/空白 / 含标点）`, cmd: 'andrea.toggleWordCountUnit' },
