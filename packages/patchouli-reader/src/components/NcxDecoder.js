...
/** —— 入口 ———————————————————————————————— */
export async function loadToc(url) {
    return url.endsWith('.ncx')
        ? fetchNcx(url) // ① 直链
        : fetchEpub(url); // ② EPub 解包
}
/* ============ ① 直接抓取 .ncx ============ */
export async function fetchNcx(ncxUrl) {
...
/* ============ ② 抓取并解包 .epub =========== */
export async function fetchEpub(epubUrl) {
...
function getNcxPath(opfXml, opfPath) {
    ...
    if (tocId) {
        ...
        if (item)
            return base + item.getAttribute('href');
    }
    // fallback：查 manifest 的 NCX MIME 类型
    const ncxItem = Array.from(doc.querySelectorAll('item')).find((i) => i.getAttribute('media-type') === 'application/x-dtbncx+xml'); // :contentReference[oaicite:9]{index=9}
    if (!ncxItem)
        throw new Error("OPF didn't declare NCX");
    return base + ncxItem.getAttribute('href');
}
function parseNcxDom(xml, baseHref) {
    const doc = new DOMParser().parseFromString(xml, 'application/xml');
    const top = Array.from(doc.querySelector('navMap').children).filter((n) => n.nodeName === 'navPoint');
    ...
    function walk(el) {
        ...
        const kids = Array.from(el.children).filter((c) => c.nodeName === 'navPoint');
        return {
...
/* ---------- A. raw data, no proxy ---------- */
export async function loadTocRaw(url) {
...
export async function loadTocDom(url) {
     ...
     */
    function renderToc(navPoints, container, onNavigate = (href) => (location.href = href)) {
     ...
     */
    function buildTocDiv(toc, onNavigate = (href) => (location.href = href)) {
...
//# sourceMappingURL=NcxDecoder.js.map