import { createChromeMcpClient, listChromeTools, callChromeTool } from "../src/mcp/chromeClient";
...
async function main() {
  ...
  try {
    const { client, disconnect } = await createChromeMcpClient({ httpUrl });

    const tools = await listChromeTools(client);
    ...
    if (toolNames.includes("open_url") || toolNames.includes("chrome_navigate")) {
      ...
      const result = await callChromeTool(client, name, { url });
